const requiredFields = ['id', 'overview', 'title', 'release_date', 'backdrop_path', 'poster_path', 'runtime'];

const watchedRequiredFields = [
  'seen', 'chosen_by', 'date_watched'
]
...
export const validateMovieData = (movie) => {
  ...
  for (let requriedField of requiredFields) {
    ...
    if(!movie['backdrop_path'].includes('.jpg') || !movie['backdrop_path'].includes('/')) {
      errors.backdrop= 'Invalid backrop path.';
  }

  if(!movie['poster_path'].includes('.jpg') || !movie['poster_path'].includes('/')) {
  ...
  if (movie['seen']) { 
    ...
    if(!/^(\d{1,2})(\/|-)(\d{1,2})(\/|-)(\d{2})$/g.test(movie['date_watched'])) {
